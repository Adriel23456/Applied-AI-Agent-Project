...
cada partida completa dentro de un único subconjunto durante la evaluación,
sin repartir sus estados entre entrenamiento, validación y prueba.
...
vistas; o si su capacidad predictiva se redujera al evaluarlo en periodos
posteriores o con mazos no observados durante el entrenamiento.
...
\textit{et al.} explican que las métricas probabilísticas evalúan dos aspectos
distintos: si los porcentajes predichos corresponden a las frecuencias
observadas (calibración) y si el modelo distingue correctamente entre estados
con mayor o menor probabilidad de victoria (refinamiento). En cambio, las
métricas basadas solo en el ordenamiento, como el área bajo la curva ROC, no
cambian cuando se transforman las probabilidades sin alterar su orden
\cite{silvafilho_calibration_2023}.

Todavía no sabemos si este patrón también aparece en los juegos de cartas.
Por eso, este trabajo comprobará si en Pokémon TCG la calibración se deteriora
antes y en mayor medida que la capacidad del modelo para distinguir entre
victorias y derrotas.

\noindent\textbf{Criterios para evaluar las hipótesis.} H1 se debilita si,
durante los primeros turnos, el modelo no obtiene un rendimiento claramente
superior al azar. También se debilita si su capacidad predictiva no mejora
conforme avanza la partida. H2 se refuta si un modelo que solo utiliza el
número de turno y la identidad del mazo funciona tan bien como el modelo que
utiliza todas las características del estado. En ese caso, la capacidad
predictiva podría deberse al turno y al mazo, y no a la información específica
de la situación de juego. H3 queda refutada si el puntaje
...
por dos razones estructurales: los estados tempranos con ventajas muy claras
son infrecuentes, y resulta más difícil relacionar un estado inicial con el
resultado final de la partida \cite{brill_winprob_2026}. En consecuencia, un
rendimiento reducido durante los primeros turnos puede tener dos explicaciones:
que el estado inicial realmente no contenga suficiente información para
predecir el resultado, o que el modelo no haya aprendido a interpretarlo bien
porque existen pocos ejemplos de esos estados.
...
\begin{enumerate}
  ...
  \item Analizar la explicabilidad y la capacidad de generalización mediante
        la eliminación de grupos de características para medir su aporte, el
        desordenamiento controlado de cada característica dentro de las
        partidas, las curvas de fiabilidad y la capacidad predictiva según el
        turno de la partida.
\end{enumerate}
...
La segunda contribución es demostrar con datos del propio corpus por qué las
partidas deben mantenerse completas en un solo subconjunto. La auditoría mide
cuántas partidas quedarían divididas entre entrenamiento y prueba si los
estados se repartieran por filas al azar. Por tanto, la partición agrupada se
justifica con una medición directa del corpus, y no solo con una afirmación
general.
