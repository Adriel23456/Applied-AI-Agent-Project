...
juego y entregaban la probabilidad de que el jugador uno venciera, puntuada
...
trata la secuencia completa de estados y acciones de la partida como un
problema de control. Esta es la formulación que se adopta aquí.
...
Estos trabajos muestran que una única métrica general puede ocultar diferencias
importantes entre las etapas de la partida: el modelo puede aportar poca
información al inicio y mucha al final. Sin embargo, no reportan la
incertidumbre de sus curvas ni analizan si el rendimiento cambia cuando cambia
la población de jugadores.
...
El contenido de estos juegos cambia con el tiempo. Bertram \textit{et al.}
estudian este problema en \emph{Magic: The Gathering} reservando un conjunto
completo de cartas para la evaluación. Sus resultados muestran una diferencia
importante: un modelo que representa cada carta únicamente por su identidad
obtiene un 67.80\,\% de acierto cuando evalúa cartas del mismo conjunto, pero
no puede hacer predicciones sobre cartas que nunca vio durante el
entrenamiento. En cambio, las representaciones construidas a partir de
características de las cartas obtienen un 68.00\,\% dentro de la distribución
y un 42.87\,\% cuando se evalúan conjuntos no vistos. Al ampliar el experimento
a un corpus de 2\,990 cartas, el mismo modelo alcanza un 55.44\,\% sobre un
conjunto reservado por completo \cite{bertram_cardrepresentations_2024}.
La lección es que un modelo puede memorizar las identidades de las cartas
conocidas y funcionar bien con ellas, pero fallar cuando aparecen cartas
nuevas. Usar características compartidas entre cartas puede ayudarle a
generalizar a contenido que no vio durante el entrenamiento.
...
puesto que sus modelos utilizan resúmenes de cada partida completa y no
estados individuales registrados durante la partida.
